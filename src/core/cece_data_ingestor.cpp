...
/**
 * @brief Default constructor for CeceDataIngestor.
 *
 * Initializes an empty data ingestor with no fields registered.
 */
CeceDataIngestor::CeceDataIngestor() {}

/**
 * @brief Destructor for CeceDataIngestor.
 *
    ...
    for (const auto& stream : config.streams) {
        for (const auto& var : stream.variables) {
            ...
            if (HasCachedField(model_name)) {
                ...
                auto it = cece_state.fields.find(model_name);
                if (it == cece_state.fields.end()) {
...
bool CeceDataIngestor::HasCachedField(const std::string& name) const {
